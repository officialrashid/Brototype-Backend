...
export const updateParticularEvents_Usecase = (dependencies: any) => {
    ...
    const executeFunction = async (reviewerId:string,eventId:string,bookedEventId:string,bookStatus:boolean) => {
        try {
            const response = await reviewerRepository.updateParticularEvents(reviewerId,eventId,bookedEventId,bookStatus);
